...
/**
 * Get the user information automatically
 *
 * @author TeacherFu
 * @version 1.0
 */
public class UserInfoAuto {
     ...
     * @return
     */
    public static int Age(int start, int end) {
        if(end <= start) {
            return 0;
        }
        int between = end - start;
        return ra.nextInt(between) + start;
    }
    ...
    public static String testPinyin(String name) throws BadHanyuPinyinOutputFormatCombination {
        ...
        for(int i = 0; i < charArray.length; i++) {
            ...
            if(Character.toString(charArray[i]).matches("[\\u4E00-\\u9FA5]+")) {
                ...
                char tmp = string.charAt(0);
                tmp -= 32;
                StringBuilder res = new StringBuilder();
                res.append(tmp);
                res.append(string.substring(1));
                pinyin.append(res.toString());
            } else {
